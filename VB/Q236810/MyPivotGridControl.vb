...
Namespace DXSample
	...
	Public Class MyFieldCustomizationTreeBox
		...
		Protected Overrides Function CreateItemViewInfo(ByVal node As PivotCustomizationTreeNode) As PivotCustomizationTreeNodeViewInfo
			If Data.ShowCustomizationTree Then
				Return New PivotCustomizationTreeNodeViewInfoTreeItem(node, Data.ActiveLookAndFeel, Data.PaintAppearancePrint.FieldHeader)
			Else
				Return New PivotCustomizationTreeNodeViewInfoListItem(node, CType(Data.ViewInfo, PivotGridViewInfo))
			End If
		End Function
	End Class
	...
	Public Class MyPivotCustomizationFieldsTree
		...
		Protected Overrides Function PopulateFields(ByVal fields As IEnumerable) As List(Of PivotCustomizationTreeNode)
			Dim result As New List(Of PivotCustomizationTreeNode)()
